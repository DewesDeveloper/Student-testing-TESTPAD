...
<body class="bg-[#f0f2f5] min-h-screen {{ $test->prevent_copy ? 'no-select' : '' }}"
      @if($test->prevent_copy) oncopy="return false" oncontextmenu="return false" @endif>
        ...
        <template x-if="!started">
            <div class="flex-1 flex items-center justify-center p-4">
                <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden border border-gray-200">
                    <div class="bg-gray-50 p-4 border-b text-center text-gray-400 text-sm italic">{{ $test->title }}</div>
                    <div class="p-10 text-center">
                        @if($test->description)
                            <h1 class="text-3xl font-light text-gray-800 mb-8 italic leading-tight">{{ $test->description }}</h1>
                        @endif

                        <div class="text-left bg-gray-50 p-6 rounded-xl border border-gray-100 mb-10 text-gray-600 italic leading-relaxed">
            ...
            <div class="flex-1 flex flex-col">
                ...
                <header class="bg-white shadow-sm sticky top-0 z-50 border-b">
                    <div class="p-4 flex justify-between items-center bg-gray-50/50">
                        <h2 class="font-bold text-gray-700 truncate mr-4 text-sm uppercase tracking-wider">{{ $test->title }}</h2>
                        @if($test->show_time || $test->limit_time)
                            <div class="bg-white px-4 py-1 rounded-full font-mono font-bold border border-orange-200 text-orange-600 shadow-sm">
                ...
                <main class="flex-1 p-4 md:p-8">
                    <form id="testForm" action="{{ route('test.submit', $test->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @foreach($questions as $index => $question)
                            <div x-show="currentQuestion === {{ $index }}" x-transition.opacity.duration.300ms class="space-y-6">
                                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-10">
                                    <div class="text-blue-500 font-black text-xs uppercase tracking-widest mb-4">
                                        Вопрос <span x-text="currentQuestion + 1"></span> из <span x-text="totalQuestions"></span>
                                    </div>
                                    <h3 class="text-xl md:text-2xl font-bold text-gray-800 mb-10 leading-snug">{{ $question->question_text }}</h3>

                                    <div class="space-y-4">
                                        <!-- ВЫБОР (Одиночный / Множественный) -->
                                        @if(in_array($question->type, ['single_choice', 'multi_choice', 'single', 'multi']))
                                            @foreach($question->options as $option)
                                                <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-blue-50 transition-all group border-gray-100"
                                                       :class="isOptionSelected({{ $question->id }}, {{ $option->id }}) ? 'border-blue-500 bg-blue-50' : ''">
                                                    <input type="{{ in_array($question->type, ['single_choice', 'single']) ? 'radio' : 'checkbox' }}"
                                                    ...
                                                    <span class="ml-4 text-gray-700 font-medium group-hover:text-blue-700">{{ $option->option_text }}</span>
                                                </label>
                                            @endforeach
                                                ...
                                                @foreach($question->options->shuffle() as $option)
                                                    <div class="flex items-center gap-4 bg-white p-4 border rounded-xl shadow-sm">
                                                        <input type="number" name="q[{{ $question->id }}][{{ $option->id }}]"
                                                               ...
                                                               class="w-14 p-2 border-2 border-blue-100 rounded-lg text-center font-bold outline-none" placeholder="№">
                                        ...
                                        @elseif($question->type === 'matching')
                                            ...
                                            <div class="space-y-3">
                                                @foreach($question->options as $option)
                                                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 border bg-gray-50 rounded-xl">
                        ...
                        @if($test->allow_comments)
                            <div x-show="currentQuestion === totalQuestions - 1" class="mt-10">
                                <div class="bg-indigo-50 border-2 border-indigo-100 rounded-2xl p-6">
                                    <label class="block text-indigo-900 font-bold mb-3 italic">💬 Ваш комментарий к тесту:</label>
                                    <textarea name="student_comment" rows="3" class="w-full p-4 border-none rounded-xl shadow-inner outline-none focus:ring-2 focus:ring-indigo-400"></textarea>
                                </div>
                            </div>
                        @endif
                    </form>
                ...
                <footer class="bg-white border-t p-6 flex justify-between items-center sticky bottom-0 z-50">
                    ...
                    <div class="flex gap-4">
                        <template x-if="currentQuestion < totalQuestions - 1">
                            <button @click="nextQuestion()" class="bg-blue-600 hover:bg-blue-700 text-white px-10 py-3 rounded-xl font-bold shadow-lg shadow-blue-200 transition-all active:scale-95">
                                Далее →
                            </button>
                        </template>
                        <template x-if="currentQuestion === totalQuestions - 1">
                            <button @click="finishTest()" class="bg-green-600 hover:bg-green-700 text-white px-10 py-3 rounded-xl font-bold shadow-lg shadow-green-200 transition-all active:scale-95">
                                Завершить
                            </button>
                        </template>
                    </div>
