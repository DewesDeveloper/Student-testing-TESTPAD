		...
		<template x-if="started">
			<div class="flex-1 flex flex-col">
				...
				<header class="bg-white shadow-sm sticky top-0 z-50 border-b">
					...
					<!-- Сетка навигации -->
					<div class="p-3 flex gap-2 overflow-x-auto nav-grid">
									...
									<div class="space-y-4">
										...
										@elseif(in_array($question->type, ['text', 'number']))
											<input type="{{ $question->type }}" name="q[{{ $question->id }}]"
												@input="markAsAnswered({{ $index }}, 'input', $el.value)"
										...
										@elseif($question->type === 'file')
											<div
												...
												<input type="file" name="q_file[{{ $question->id }}]"
													@change="markAsAnswered({{ $index }}, 'input', $el.value)"
										...
										@elseif($question->type === 'sequence')
											<div class="space-y-3">
												<p class="text-xs text-gray-400 uppercase font-bold mb-2 italic">Укажите
													порядковые номера (1, 2, 3...):</p>
												@foreach($question->options->shuffle() as $option)
													<div
														...
														<input type="number" name="q[{{ $question->id }}][{{ $option->id }}]"
															...
															class="w-14 p-2 border-2 border-blue-100 rounded-lg text-center font-bold focus:border-blue-500 outline-none"
															placeholder="№">
										...
										@elseif($question->type === 'matching')
											{{-- Получаем список всех ПРАВЫХ частей (match_text) для этого вопроса --}}
											@php
												$matches = $question->options->pluck('match_text')->filter()->unique()->shuffle();
											@endphp

											{{-- Если список пуст, выводим подсказку для отладки --}}
											@if($matches->isEmpty())
												<div class="p-4 bg-red-50 text-red-500 rounded-lg text-xs italic">
													Ошибка: Преподаватель не заполнил правые части соответствий.
												</div>
											@endif
							...
							<div x-show="currentQuestion === totalQuestions - 1" x-transition class="mt-10">
								<div class="bg-indigo-50 border-2 border-indigo-100 rounded-2xl p-6">
									<label class="block text-indigo-900 font-bold mb-3 flex items-center gap-2">
										<span>💬</span> Комментарий к тесту (необязательно)
									</label>
									<textarea name="student_comment" rows="3"
										class="w-full p-4 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none shadow-inner"
										placeholder="Ваши замечания, вопросы или пожелания..."></textarea>
								</div>
							</div>
						@endif
					</form>
	...
	<script>
		function testApp() {
			return {
				...
				selectionMap: {}, // Для подсветки активных опций {question_id: [option_ids]}
				...
				startTest() {
					this.started = true;
					// Запускаем таймер только если включена настройка времени
				...
				markAsAnswered(index, type, qId, optId = null) {
					if (type === 'choice') {
						// Логика подсветки и отметки
						const radios = document.querySelectorAll(`input[name="q[${qId}]"]`);
						const isMulti = radios[0].type === 'checkbox';

						if (!this.selectionMap[qId]) this.selectionMap[qId] = [];

						if (isMulti) {
							const checked = Array.from(document.querySelectorAll(`input[name="q[${qId}][]"]:checked`)).map(i => parseInt(i.value));
							this.selectionMap[qId] = checked;
							this.answeredFlags[index] = checked.length > 0;
						} else {
							this.selectionMap[qId] = [optId];
							this.answeredFlags[index] = true;
						}
					} else if (type === 'input') {
						this.answeredFlags[index] = qId.trim().length > 0;
					} else if (type === 'sequence' || type === 'matching') {
						// Проверяем, заполнены ли все поля в блоке
						const inputs = document.querySelectorAll(`[name^="q[${qId}]"]`);
						const allFilled = Array.from(inputs).every(i => i.value.trim() !== "");
						this.answeredFlags[index] = allFilled;
					}
				...
				finishTest(force = false) {
					if (!force && {{ $test->require_all_answers ? 'true' : 'false' }}) {
						...
						if (count < this.totalQuestions) {
							alert('Вы должны ответить на ВСЕ вопросы перед завершением!');
							return;
						}
					}

					if (force || confirm('Вы действительно хотите завершить тестирование?')) {
