...
@section('header_title', 'Редактор теста: ' . $test->title)

@section('content')
	<div x-data="testEditor()" x-init="loadInitialData()" class="relative flex gap-6 items-start">

		<!-- ЛЕВАЯ ЧАСТЬ: КОНСТРУКТОР -->
		<div class="flex-1 min-w-0">
			<form action="{{ route('tests.update', $test->id) }}" method="POST" id="mainForm" enctype="multipart/form-data">
				@csrf
				@method('PUT')
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							<div class="flex gap-2">
								<select name="discipline_id"
									...
									@foreach($disciplines as $d)
										<option value="{{ $d->id }}" {{ $test->discipline_id == $d->id ? 'selected' : '' }}>
											{{ $d->name }}
										</option>
									@endforeach
								</select>
						...
						<div>
							...
							<textarea name="description" rows="1"
								...
								placeholder="О чем этот тест?">{{ $test->description }}</textarea>
						</div>
					</div>
				</div>
				...
				<div class="space-y-8">
					<template x-for="(question, qIndex) in questions" :key="qIndex">
						<div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden"
							...
							<div class="p-6">
								<!-- ВКЛАДКА: РЕДАКТОР -->
								<div x-show="tab === 'editor'" x-transition>
									<div
									...
									<div class="border rounded-lg p-4 mb-6 bg-gray-50">
										<textarea :name="`questions[${qIndex}][text]`" x-model="question.text" required
											rows="3"
											class="w-full bg-transparent outline-none text-gray-700 text-lg"></textarea>
									...
									<div
										class="bg-[#7e8e9b] text-white text-[10px] uppercase font-bold px-3 py-1.5 inline-block mb-4 relative">
										<span x-text="getOptionTitle(question.type)"></span>
										<div
											class="absolute right-[-10px] top-0 border-y-[12px] border-y-transparent border-l-[10px] border-l-[#7e8e9b]">
										</div>
									</div>
									...
									<!-- ВКЛАДКА: КОММЕНТАРИЙ -->
									<div x-show="tab === 'comment'" x-transition>
										<textarea :name="`questions[${qIndex}][explanation]`" x-model="question.explanation"
									...
									<div x-show="tab === 'params'" x-transition>
										<div class="space-y-3 p-4 text-sm text-gray-600">
											<label class="flex items-center gap-3 cursor-pointer"><input type="checkbox"
													...
													:checked="question.is_required" class="w-4 h-4 text-blue-600">
												<span>Обязательный вопрос</span></label>
											<label class="flex items-center gap-3 cursor-pointer"><input type="checkbox"
													:name="`questions[${qIndex}][shuffle_options]`"
													:checked="question.shuffle_options" class="w-4 h-4 text-blue-600">
												<span>Перемешивать ответы</span></label>
										</div>
									</div>
									...
									<div class="mt-6 pt-6 border-t flex justify-between items-center">
										<div class="flex items-center gap-2">
											...
											<input type="number" :name="`questions[${qIndex}][points]`"
												...
												class="w-16 border rounded p-1 text-center font-bold text-blue-600 shadow-inner">
										</div>
										<button type="button" @click="removeQuestion(qIndex)"
											class="text-xs text-red-500 font-bold uppercase hover:underline">Удалить
											вопрос</button>
									</div>
								</div>
							</div>
				...
				<!-- КНОПКИ -->
				<div class="mt-12 p-6 bg-white rounded-xl border shadow-xl flex justify-end gap-4 sticky bottom-6 z-40">
					<a href="{{ route('tests.show', $test->id) }}"
						class="px-8 py-3 text-gray-400 font-bold hover:text-gray-600">ОТМЕНА</a>
					<button type="submit"
						class="bg-blue-600 hover:bg-blue-700 text-white px-12 py-3 rounded-xl font-black shadow-lg transition-all active:scale-95">
						💾 СОХРАНИТЬ ИЗМЕНЕНИЯ
					</button>
				</div>
			</form>
		...
		<div class="w-72 sticky top-24 space-y-1">
			<div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-2 italic">Добавить ещё вопрос
			</div>

			<template x-for="type in types" :key="type.id">
				<button type="button" @click="addQuestion(type.id)"
					class="w-full flex items-center justify-between p-4 bg-white border border-gray-100 hover:bg-blue-50 transition-all text-sm text-gray-600 group rounded-lg shadow-sm">
					<span class="flex items-center gap-3">
						<span x-text="type.icon"></span>
						<span x-text="type.name"></span>
					</span>
					<span class="text-blue-400 font-bold text-lg opacity-0 group-hover:opacity-100">+</span>
				</button>
			</template>
		</div>

		<!-- МОДАЛКА ДИСЦИПЛИНЫ -->
		<div x-show="showModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" x-cloak>
	...
	<script>
		function testEditor() {
			return {
				...
				loadInitialData() {
					// Загружаем данные из PHP в Alpine
					const rawData = @json($test->questions->load('options'));
					this.questions = rawData.map(q => ({
						type: q.type,
						text: q.question_text,
						points: q.points,
						explanation: q.explanation || '',
						is_required: q.is_required == 1,
						shuffle_options: q.shuffle_options == 1,
				...
				addQuestion(type) {
					...
					this.questions.push({ type, text: '', points: 1, explanation: '', is_required: false, shuffle_options: false, options });
				...
				getOptionTitle(type) {
					if (['text', 'number'].includes(type)) return 'Правильные ответы (Ключи)';
					if (type === 'matching') return 'Пары соответствий';
					if (type === 'sequence') return 'Элементы в правильном порядке';
