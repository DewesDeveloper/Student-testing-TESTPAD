...
<body>
	...
	<div class="result-badge">
		Итоговый результат: <span
			class="score-big">{{ round(($result->score / ($result->total_points ?: 1)) * 100) }}%</span><br>
	...
	@foreach($test->questions as $index => $question)
		...
		<div class="question-block">
			...
			<div class="options-list">
				<!-- А: Одиночный и множественный выбор -->
				@if(in_array($question->type, ['single_choice', 'multi_choice', 'single', 'multi']))
					...
					<!-- Б: Соответствия (Matching) - ИСПРАВЛЕНО -->
				@elseif($question->type === 'matching')
					<div style="margin-left: 20px;">
						@foreach($question->options as $option)
							@php
								$studentChoice = $studentAns[$option->id] ?? 'не выбрано';
								$isCorrect = $studentChoice === $option->match_text;
							@endphp
							<div class="option {{ !$isCorrect ? 'incorrect' : 'correct' }}"
								style="border-bottom: 1px solid #f0f0f0;">
								<b>{{ $option->option_text }}</b>
								<span style="color: #999;"> &rarr; </span>
								<span>{{ $studentChoice }}</span>
								@if(!$isCorrect)
									<div style="font-size: 10px; color: #666;">(Правильно: {{ $option->match_text }})</div>
								@endif
							</div>
						@endforeach
					</div>

					<!-- В: Последовательность (Sequence) - ИСПРАВЛЕНО -->
					...
					<div style="margin-left: 20px;">
						@foreach($question->options as $idx => $option)
							@php
								...
								$correctRank = $idx + 1;
								$isCorrect = $studentRank == $correctRank;
							@endphp
							<div class="option {{ !$isCorrect ? 'incorrect' : 'correct' }}">
								№{{ $studentRank }} - {{ $option->option_text }}
								@if(!$isCorrect) (Правильно: №{{ $correctRank }}) @endif
							</div>
						@endforeach
					</div>
				...
				@elseif($question->type === 'file')
					<div class="option picked">
						<strong>Файл:</strong> {{ $studentAns['name'] ?? 'не загружен' }}
					</div>

					<!-- Д: Свободная форма / Текст -->
				@else
					<div class="option picked">
						<strong>Ответ:</strong> {{ is_array($studentAns) ? 'ошибка данных' : ($studentAns ?: 'нет ответа') }}
					</div>
